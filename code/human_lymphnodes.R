...
# Notes on data:
# No annotations provided. Integrate batch effects.
...
so.anchors <- FindIntegrationAnchors(object.list = so.list, dims = 1:20, k.filter=100, anchor.features = vargenes, reference = reference_dataset)
so.integrated <- IntegrateData(anchorset = so.anchors, dims = 1:20, features.to.integrate = rownames(so))
...
marker_genes <- c("ACKR4", "NUDT4", "NTS", "CCL20", "CXCL5", "LYZ", "DARC", "MARCO", "THY1", "TNFRSF9", "MFAP4", "LOX", "LTB", "PDPN", "CCL21", "SCG3", "CLEC4G", "CLEC4M", "LGMN", "CD209", "SPARCL1", "PLVAP", "CCL19", "RGS5")
