...
# Notes on data:
# We downsample to 60,000 samples.
# Cluster labels are provided. There are a lot.

#------------------------------------------------LOAD AND FORMAT DATA-----------------------------------------------#

# Read in data and annotations
library(R.matlab)
file <- paste0("../data/", name, "/GSM3017261_150000_CNS_nuclei.mat")
all_dat <- readMat(file)
mat <- all_dat$DGE %>%
  Matrix(sparse = TRUE) %>%
  as("dgCMatrix") %>%
  t()
rows <- all_dat$genes
cols <- all_dat$barcodes
rownames(mat) <- lapply(rows[,1], trimws)
colnames(mat) <- lapply(1:dim(mat)[2], toString)
mat.annot <- all_dat$cluster.assignment
rownames(mat.annot) <- lapply(1:dim(mat)[2], toString)
mat.annot <- data.frame(mat.annot, stringsAsFactors=FALSE)
colnames(mat.annot) <- c("cluster_id")
...
PlotAndSaveUMAPClusters(so, so@meta.data$cluster_id, name, suffix = "_pre_def")
...
marker_genes <- c("MEG3", "MBP", "PDGFRA", "DOCK2", "RGS5", "COL1A2", "ALDH1L1", "DNAH11", "MYBPC1", "DEPTOR", "RARB", "SATB2", "TFAP2D", "FIGN", "ARAP1", "PAX3", "NTN1", "PAX2", "SLC6A3", "FN1", "TSPAN18", "PDE11A", "DLX6OS1")
...
Idents(object=so) <- "cluster_id"
clus <- unique(so@meta.data$cluster_id)
demarkers_pre_def <- WithinClusterFeatures(so, "cluster_id", clus, name, suffix = "_pre_def")
...
PlotAndSaveDEGenesOnUMAP(so, demarkers_pre_def, name, suffix = "_pre_def", height = 60, rank_by_tstat = TRUE, raster_dpi = 50)

# Save Seurat object
saveRDS(so, paste0("../data/", name, "/so.rds"))m
