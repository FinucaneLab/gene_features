...
file <- paste0("../data/", name, "/droplet_filtered_matrix.mtx")
mat <- readMM(file) %>%
  Matrix(sparse = TRUE) %>%
  as("dgCMatrix")
file <- paste0("../data/", name, "/droplet_filtered_genes.tsv")
rows <- data.frame(fread(file, header=T), row.names=1)
file <- paste0("../data/", name, "/droplet_filtered_cells.tsv")
