...
file <- paste0("../data/", name, "/matrix.mtx")
mat <- readMM(file) %>%
  Matrix(sparse = TRUE) %>%
  as("dgCMatrix")
file <- paste0("../data/", name, "/genes.tsv")
rows <- data.frame(fread(file, header=F), row.names=1)
file <- paste0("../data/", name, "/barcodes.tsv")
...
marker_genes <- c("CALB1", "PGAM2", "PVALB", "UMOD", "EGF", "PROX1", "PSCA", "CLDN4", "PAX2", "JAG1", "FST", "ABCC3", "LRP2", "CP", "CLDN5", "MSMP", "NPHS2", "ELV1", "SPP2", "CYP51")
