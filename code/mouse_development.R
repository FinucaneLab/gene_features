...
# Notes on data:
# Data is large (> 1000000 cells) so we downsample to 100000
# Cell-type annotations are provided; we use those


#------------------------------------------------LOAD AND FORMAT DATA-----------------------------------------------#

# Read in data and annotations
file <- paste0("../data/", name, "/cds_cleaned_sampled_100k.RDS")
library(monocle)
raw <- readRDS(file)
mat <- raw@assayData$exprs
# ENSMUSG names have weird suffixes; we subset the names to the first 18 characters (i.e. length of ENSMUSG ID)
...
file <- paste0("../data/", name, "/cell_annotate.csv")
mat.annot <- data.frame(fread(file), row.names = 1)
mat.annot <- mat.annot %>% dplyr::select(Main_cell_type)
mat.annot <- subset(mat.annot, rownames(mat.annot) %in% colnames(mat))
...
PlotAndSaveUMAPClusters(so, so@meta.data$Main_cell_type, name, suffix = "_pre_def")
...
marker_genes <- c("COL6A6", "GLIS1", "NR1H5", "COL9A1", "NTNG1", "TRP63", "PTH2R", "FNDC3C1", "MYBL1", "TFAP2D", "DMBX1", "MYLK4", "FOXB1", "NPY", "RAB5A")
...
Idents(object=so) <- "Main_cell_type"
clus <- unique(so@meta.data$Main_cell_type)
demarkers_pre_def <- WithinClusterFeatures(so, "Main_cell_type", clus, name, suffix = "_pre_def")
