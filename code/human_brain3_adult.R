...
# Notes on data:
# Cell types and subtypes are provided; we'll use the subtypes as the pre-defined clusters

#------------------------------------------------LOAD AND FORMAT DATA-----------------------------------------------#

# Read in data and annotations
file <- paste0("../data/", name, "/Sestan.adultHumanNuclei.Psychencode.Rdata")
load(file)
mat <- umi.raw %>%
  data.matrix() %>%
  Matrix(sparse = TRUE)
...
PlotAndSaveUMAPClusters(so, so@meta.data$subtype, name, suffix = "_pre_def")
...
Idents(object=so) <- "subtype"
clus <- unique(so@meta.data$subtype)
demarkers_pre_def <- WithinClusterFeatures(so, "subtype", clus, name, suffix = "_pre_def")
