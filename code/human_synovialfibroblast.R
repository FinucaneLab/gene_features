...
file <- paste0("../data/", name, "/gene_sorted-exprs_human.mtx.gz")
mat <- readMM(file) %>%
  Matrix(sparse = TRUE) %>%
  as("dgCMatrix")
file <- paste0("../data/", name, "/exprs_human_genes.tsv.gz")
rows <- data.frame(fread(file, header=F), row.names=1)
file <- paste0("../data/", name, "/exprs_human_barcodes.tsv.gz")
...
mat.annot <- data.frame(fread(file, header=F, skip=2), row.names=1)
colnames(mat.annot) <- colnames(data.frame(fread(file, header=T), row.names=1))
...
PlotAndSaveUMAPClusters(so, so@meta.data$cell_subtype, name, suffix = "_pre_def")
...
marker_genes <- c("VWF", "PRG4", "THY1", "MCAM", "CD146", "CD90", "CD55", "GGT5", "PDPN", "MKI67", "CD146", "CD31")
...
Idents(object=so) <- "cell_subtype"
clus <- unique(so@meta.data$cell_subtype)
clus <- clus[!is.na(clus)]
demarkers_pre_def <- WithinClusterFeatures(so, "cell_subtype", clus, name, suffix = "_pre_def")
