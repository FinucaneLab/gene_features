...
# Notes on data:
# Organ labels provided. For this dataset, have to convert from AnnData object to .mtx format using Python
...
file <- paste0("../data/", name, "/190304_maca_bulk_matrix.mtx")
mat <- readMM(file) %>%
  Matrix(sparse = TRUE) %>%
  as("dgCMatrix")
file <- paste0("../data/", name, "/190304_maca_bulk_genes.tsv")
rows <- data.frame(fread(file, header=T), row.names=1)
file <- paste0("../data/", name, "/190304_maca_bulk_cells.tsv")
...
PlotAndSaveUMAPClusters(so, so@meta.data$Organ, name, suffix = "_pre_def")
...
Idents(object=so) <- "Organ"
clus <- unique(so@meta.data$Organ)
demarkers_pre_def <- WithinClusterFeatures(so, "Organ", clus, name, suffix = "_pre_def")
