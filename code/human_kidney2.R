...
# Notes on data:
# Data is very large. We downsample to 60,000 cells.
# Annotations provided, but I don't see any cluster labels.

#------------------------------------------------LOAD AND FORMAT DATA-----------------------------------------------#

# Read in data and annotations
file <- paste0("../data/", name, "/matrix.mtx.gz")
mat <- readMM(file) %>%
  Matrix(sparse = TRUE) %>%
  as("dgCMatrix")
file <- paste0("../data/", name, "/genes.tsv.gz")
...
marker_genes <- c("CD52", "ITGAX", "HLA-DRA", "CD1C", "XCR1", "CLEC9A", "THBD", "CD14", "S100A8", "S100A9", "MRC1", "C1QC", "CD163", "FCGR3A", "SAA1", "SAA2", "LCN2", "SLPI", "S100A9", "S100P", "DUOX2", "MUC4")
