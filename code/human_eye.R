...
# Notes on data:
# Cell type and cluster annotations provided. We use the cluster annotations, because they appear more fine-grained.
...
filelist = Sys.glob(paste0("../data/", name, "/GSM*"))
...
# Read in annotations
file = paste0("../data/", name, "/GSE135133_clusterAssignments.txt.gz")
...
so@meta.data$Clusters_full <- as.character(so@meta.data$Clusters_full)
PlotAndSaveUMAPClusters(so, so@meta.data$Clusters_full, name, suffix = "_pre_def")
...
marker_genes <- c("GAD1", "GAD2", "SLC6A9", "GFAP", "VSX2", "GRM6", "GRIK1", "ARR3", "ONECUT2", "HTRA1", "RLBP1", "GLUL", "CD163", "RBPMS", "RHO", "BEST1", "FOXO1")
...
Idents(object=so) <- "Clusters_full"
clus <- unique(so@meta.data$Clusters_full)
clus = clus[!is.na(clus)]
demarkers_pre_def <- WithinClusterFeatures(so, "Clusters_full", clus, name, suffix = "_pre_def")
