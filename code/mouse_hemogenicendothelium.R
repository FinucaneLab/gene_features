...
# Parameters
name <- "mouse_hemogenicendothelium"
number_pcs <- 60
vargenes <- 2000
clus_res <- 0.6
...
# Notes on data:
# Cell type labels are provided, so we use these

#------------------------------------------------LOAD AND FORMAT DATA-----------------------------------------------#

# Read in data and annotations
file <- paste0("../data/", name, "/GSE137116_gene_by_cell_count_matrix.txt.gz")
mat <- readMM(file) %>%
  data.matrix() %>%
  Matrix(sparse = TRUE)
file <- paste0("../data/", name, "/GSE137116_cell_annotation.csv.gz")
col.annot <- data.frame(fread(file, header=T), row.names=1)
file <- paste0("../data/", name, "/GSE137116_gene_annotation.csv.gz")
row.annot <- data.frame(fread(file, header=T), row.names=1)
...
PlotAndSaveUMAPClusters(so, so@meta.data$Cell_type_refined, name, suffix = "_pre_def")
...
marker_genes <- c("B220", "CD19", "MAC1", "GR1", "CD45", "EFNB2", "PTPRC", "CD44", "EYA2", "PROCR", "CD27", "MECOM", "HEY2", "CD45", "CD90", "CD25")
...
Idents(object=so) <- "Cell_type_refined"
clus <- unique(so@meta.data$Cell_type_refined)
### Issue with NAs; fix by converting NAs to anything else
clus <- clus[!is.na(clus)]
so@meta.data$Cell_type_refined[is.na(so@meta.data$Cell_type_refined)] <- "NA"
demarkers_pre_def <- WithinClusterFeatures(so, "Cell_type_refined", clus, name, suffix = "_pre_def")
